...
def is_not_root_node(context):
    return context['node'].parent is not None  # TODO: use node's proper method to determine if it's not a root node


def is_not_instance_root_node(context):
    return context['object'].parent is not None # TODO: use node's proper method to determine if it's not a root node
