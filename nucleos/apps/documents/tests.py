...
TEST_DOCUMENT_PATH = os.path.join(settings.BASE_DIR, 'contrib', 'sample_documents', 'nucleos_11_1.pdf')
TEST_SIGNED_DOCUMENT_PATH = os.path.join(settings.BASE_DIR, 'contrib', 'sample_documents', 'nucleos_11_1.pdf.gpg')
...
class DocumentTestCase(TestCase):
    def setUp(self):
        ...
        with open(TEST_DOCUMENT_PATH) as file_object:
            new_version = self.document.new_version(file=File(file_object, name='nucleos_11_1.pdf'))
    ...
    def test_document_creation(self):
        ...
        self.failUnlessEqual(self.document.file_filename, 'nucleos_11_1.pdf')
        ...
        with open(TEST_SIGNED_DOCUMENT_PATH) as file_object:
            ...
            new_version = self.document.new_version(file=File(file_object, name='nucleos_11_1.pdf.gpg'), **new_version_data)
...
class DocumentSearchTestCase(TestCase):
    ...
    def test_simple_search_after_related_name_change(self):
        ...
        model_list, flat_list, shown_result_count, result_count, elapsed_time = document_search.simple_search('nucleos')
...
class DocumentAPICreateDocumentTestCase(TestCase):
    ...
    def test_uploading_a_document_using_token_auth(self):
        ...
        self.failUnlessEqual(document.file_filename, 'nucleos_11_1.pdf')
...
class DocumentsViewsFunctionalTestCase(TestCase):
    ...
    def test_document_view(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue('Details for: nucleos_11_1.pdf' in response.content)

        # test document advanced view
        response = self.client.get(reverse('document_view_advanced', args=[self.document.pk]))
        self.assertEqual(response.status_code, 200)
        self.assertTrue('Document properties for: nucleos_11_1.pdf' in response.content)
